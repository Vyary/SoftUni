...
while not obtained:
    ...
    for index in range(0, len(loot), 2):
        number_of_shards = int(loot[index])
        type_of_shards = loot[index + 1].lower()
        if type_of_shards not in inventory:
            inventory[type_of_shards] = 0
        inventory[type_of_shards] += number_of_shards
        if inventory['shards'] >= shards_needed:
            print("Shadowmourne obtained!")
            inventory['shards'] -= shards_needed
            obtained = True
        elif inventory['fragments'] >= shards_needed:
            print("Valanyr obtained!")
            inventory['fragments'] -= shards_needed
            obtained = True
        elif inventory['motes'] >= shards_needed:
            print("Dragonwrath obtained!")
            inventory['motes'] -= shards_needed
            obtained = True
        if obtained:
            break
