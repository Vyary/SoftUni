...
class Group:
    ...
    def __add__(self, other: "Group") -> "Group":
        ...
        new_list_of_people = self.people + other.people

        new_group = Group(new_name, new_list_of_people)
        return new_group
...
"""                     Task:
Create a class called Person. Upon initialization, it will receive a name (str) and
a surname (str). Implement the needed magic methods so that:
•	Each person could be represented by their names, separated by a single space.
•	When you concatenate two people, you should return a new instance of a person who
will take the first name from the first person and the surname from the second person.
...
•	When you access the length of a group instance, you should receive the total number
