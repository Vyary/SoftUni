class Vet:
    ...
    def info(self):
        remaining_space = Vet.space - len(Vet.animals)
        return (
            f"{self.name} has {len(self.animals)} animals."
            f"{remaining_space} space left in clinic"
        )
...
There should also be 2 class attributes: animals (empty list) which will store the total
amount of animals for all vets; and space (5 by default). You should create 3 additional
instance methods:
...
-	info()
o	Returns info about the vet, the number of animals in his list and the free space in
the clinic:
