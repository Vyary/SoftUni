...
def dictionary_replace(dictionary, replace):
...
while vowels and consonants:
    ...
    dictionary_replace(flowers, current_vowel)
    dictionary_replace(flowers, current_consonant)
...
from the consonants collection. Then check if these letters are present in one or more
of the given words. If a letter is present, that part of the word is considered found.
The word is gradually revealed with each letter found. Continue processing the next
couple of letters until you find one of the given words above.
A letter (vowels or consonants) could participate in more than one word or more than
one time in a word, for example:
...
in the format described below. Otherwise, print "Cannot find any word!" on the first
line and the remaining letters in each sequence in the format described below.
...
•	On the next lines, print the remaining letters in each collection
