...
# a series of Jump commands will follow until you receive "Love!". Every house in the neighborhood needs a
# certain number of hearts delivered by Cupid so it can celebrate Valentine's day. The integers in the '
...
# •	If Cupid jumps to a house where the needed hearts are already 0, print on the
# console "Place {house_index} already had Valentine's day."
...
# He will end up at index 2 and decrease the needed hearts by 2: [6, 6, 4]. Next, he jumps again with a length of 2 and
# goes outside the neighborhood, so he goes back to the first house (index 0) and
...
while True:
    ...
    jump_amount = int(split_command[1])
    current_position += jump_amount
    while current_position > len(neighborhood) - 1:
        current_position = 0
